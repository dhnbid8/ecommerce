...
def get_month_payment():
    ...
    orders = Order.objects.filter(created_at__month=tomonth)
    payment = 0
    for order in orders :
        payment += order.paid_amount
...
def get_month_orders_count():
    tomonth = datetime.now().month
    orders = Order.objects.filter(created_at__month=tomonth, status='PE').all()
    
    
    return orders.count()

@register.simple_tag
def get_month_sells_count():
    tomonth = datetime.now().month
    orders = Order.objects.filter(created_at__month=tomonth, status='PO').all()
